...
while (restartGame) {
    ...
    // Continues looping until the user guesses the correct number of runs out of attempts (Loops until a BREAK keyword is run)
    while (true) {
        if (guess === `Boogie`){
  ...
  The number was ${randomNum}`);
            ...
            // Checks if user's guess was too high and prompts user to guess again if guess was hig low
        } else {
    ...
    playAgain = prompt(`Would you you like to play again? Y for yes. N for no.`)

    // Loop continues until user submits a valid response
    while (true) {
        // Checks if the user's anser is no (AKA "N" or "n")
        if (playAgain.toUpperCase() === "N") {
            // Alerts the user that the game is ove rand game does not start
            alert(`Thanks for playing`);
            restartGame = false;
            break;
            // Checks if the user's anser is yes (AKA  "Y" or "y")
